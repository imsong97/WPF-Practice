-- 환자 프로시저
--
IF EXISTS (SELECT name FROM sys.objects WHERE name = 'WEB_ADMIN_HRDBFROMACCESSCONTROL_SCHEDULER')
BEGIN
	DROP PROCEDURE WEB_ADMIN_HRDBFROMACCESSCONTROL_SCHEDULER
END
GO
CREATE PROCEDURE [dbo].[WEB_ADMIN_HRDBFROMACCESSCONTROL_SCHEDULER]
...
BEGIN 
	...
	AND	   obj.name = 'WEB_ADMIN_HRDBFROMACCESSCONTROL_SCHEDULER';
	IF (@CNTPROC>1) 
	BEGIN
	...
	-- 임시 HRPersonTable2 테이블 생성
	CREATE TABLE #tmpHRPerson2_0(
	...
	CREATE TABLE #TMP(
		...
		OrgCode			nvarchar(50),
		OrgName			nvarchar(50),
		Personststus	nvarchar(50)
	)

	-----------------------------------------------
	-- 임시 테이블에 저장된 데이터 HRPerson2에 저장
	-----------------------------------------------
-------------------------------------------------------------
	INSERT INTO HRPersonTable2
	(
		[Sabun]
		,[Name]
		,[CardNo]
 	...
 	SELECT  Sabun as Sabun, 
			Name as Name,            
			'4' + dbo.fnPatientToUserCardNo(Sabun) + '1000' as CardNo,           
			dbo.fnPatientToUserCardNo(Sabun) as UserCardNo,
			null as Tel,
			null as Mobile,
			...
			'환자' as GradeName,
			0 as PersonType,
			PersonStatus as PersonStatus,
			0 as CardStatus,
			0 as IssuCnt,
			CONVERT(datetime, ValidDate, 120) + '23:59:59' as ValidDate,
			GETDATE() as InsertDate
	FROM OPENQUERY(M2, 'select * from PUB.HRPERSONTABLE2')
	WHERE Sabun NOT IN(SELECT P.Sabun FROM OPENQUERY(M2, 'select * from PUB.HRPERSONTABLE2') M
														INNER JOIN Person P ON M.Sabun = P.Sabun 
														INNER JOIN Org O ON P.OrgID = O.OrgID
												WHERE P.PersonStatusID = 0 and M.OrgName = O.OrgName and P.ValidDate = (CONVERT(datetime, M.ValidDate, 120) + '23:59:59'))

	INSERT INTO #tmpHRPerson2_0
	(
		[Sabun]
		,[Name]
		,[CardNo]
 	...
 	SELECT  Sabun, ISNULL(Name,''),CardNo,UserCardNo,Tel,Mobile, ParentOrgCode, 
		...
		-- 임시테이블 마지막 ROWNUM 추출
	SELECT @tmpHRPersonMaxCnt = COUNT(*) FROM #tmpHRPerson2_0
...
--1차적으로 중복되는 레코드는 빼고 삽입한다.	
	...
	BEGIN
		BEGIN TRY
			...
			IF ( NOT EXISTS (SELECT * FROM #tmpHRPerson2_1 WHERE 
					Sabun = (SELECT TOP(1) Sabun FROM #tmpHRPerson2_0 WHERE Seq = @HRPersonWhileCnt) ) )
			BEGIN
				INSERT INTO #tmpHRPerson2_1
				(
				...
				SELECT  					
					...
					[GradeName],[PersonType],[PersonStatus],[CardStatus],
					[IssuCnt],[ValidDate],[InsertDate]
				FROM #tmpHRPerson2_0 WHERE Seq = @HRPersonWhileCnt
			END
			ELSE
			BEGIN
				SET @TestFlag=1
		...
		BEGIN CATCH
			...
			VALUES ( GETDATE() , 'ERROR #tmpHRPerson2_0 : ' +  ERROR_MESSAGE() )
		END CATCH
...
------------------------------------------------------------------------------------ 
	...
	SELECT @tmpHRPersonMaxCnt = COUNT(*) FROM #tmpHRPerson2_1 -- 임시테이블 마지막 ROWNUM 추출
	/* 기기권한용 */
	--------------------------------------------------------------------------------------------
	WHILE(@HRPersonWhileCnt <= @tmpHRPersonMaxCnt)
	BEGIN
		BEGIN TRY
			...
			SELECT @Sabun = Sabun, @Name = Name, @CardNo = CardNo, @Tel = Tel, @Mobile = Mobile, @OrgName = OrgName
				...
				FROM #tmpHRPerson2_1 WHERE Seq = @HRPersonWhileCnt
...
--------------------------------------------------------------------------------	
			...
			-- 신규 그룹ID 구함. GroupComment 에 팀코드 넣음.@PatientGroup  OR @OrgNamee
			select @NewGroupID = PersonGroupID from PersonGroup where GroupName = @PatientGroup
			IF ( @NewGroupID is null ) -- 그룹이 없으면 그룹생성
			BEGIN
				Insert into PersonGroup (GroupName,GroupComment,InsertDate,UpdateID,UpdateIP)
					values (@PatientGroup, @PatientGroup, GETDATE(), 0, 'HR')
				select @NewGroupID = PersonGroupID from PersonGroup where GroupName = @PatientGroup				
			END
--------------------------------------------------------------------------------	
			...
			declare @tmpcardstatus int = (select cardstatusid from card c inner join person p on c.pid = p.pid where p.sabun = @sabun)
			-----------------------------------------------------------------------------------------------------------------------------
...
---------------------------------------------------------------------------------------------------		
			...
			BEGIN
				...
				SET Name = b.Name
					...
					,	UpdateDate = GETDATE()
					,	UpdateID = 0
				FROM Person a JOIN #tmpHRPerson2_1 b
				ON a.Sabun = b.Sabun
			...
			BEGIN
				...
				FROM #tmpHRPerson2_1 WHERE Seq = @HRPersonWhileCnt AND PersonStatus=0
			END
---------------------------------------------------------------------------------------------------		
-- 2. Card 추가 및 수정
---------------------------------------------------------------------------------------------------		
					...
					BEGIN
						INSERT INTO 
							...
							SELECT @CardNo, (SELECT PID FROM Person WHERE Sabun = tp.Sabun), 
								   ...
								   CardUser1, CardUser2, CardUser3, CardUser4, CardUser5 , @UserCardNo 
							FROM #tmpHRPerson2_1 tp WHERE Seq = @HRPersonWhileCnt AND CardStatus=0
					END
				END
				ELSE
				BEGIN
					...
					BEGIN
						...
						FROM card c, ( select top 1 * from #tmpHRPerson2_1 WHERE Seq = @HRPersonWhileCnt ) tp						
...
---------------------------------------------------------------------------------------------------		
			...
			--		select @OldGroupID = PersonGroupID from PersonGroup where GroupName = (select OrgName from org where OrgID = @PreOrgNameID)
					--                   조직별 출입그룹설정
					IF (@OldGroupID IS NULL OR @NewGroupID <> @OldGroupID or @tmppersonstatus <> @PersonStatus or @tmpcardstatus <> @CardStatus or @OldValidDate <> @ValidDate)
					BEGIN
						-- 이전 그룹 권한 삭제
...
--------------------------------------------------------------------------------------------------
-- 4. HRPersonTable2 임시 데이터 삭제
---------------------------------------------------------------------------------------------------		
			DELETE HRPersonTable2 WHERE Sabun = @Sabun
		END TRY
		BEGIN CATCH
			INSERT INTO ServiceLog
			(
			...
			VALUES ( GETDATE() , 'ERROR HRPersonTable2 : ' +  ERROR_MESSAGE() )
		...
		SET @HRPersonWhileCnt = @HRPersonWhileCnt + 1
	END
	DROP TABLE #tmpHRPerson2_1
	DROP TABLE #tmpHRPerson2_0
	DROP TABLE #tmp
END
GO